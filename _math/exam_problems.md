...
<https://mathkonspekt.blogspot.com/2014/02/0.html>
<http://blog.boriskin.me/post/24678682034/задачи-в-шад-и-их-решения>
...
Пусть $A_1, A_2, \ldots, A_n$ --- конечные множества и $a_{ij} = \vert A_i \cap A_j \vert$. Докажите, что матрица $(a_{ij})_{i=1,2,\ldots,n}^{j=1,2,\ldots,n}$ неотрицательно определена.

<https://github.com/efiminem/supershad/issues/7>
...
$$S_M = \left\{ \left. \frac{x+y}{2} \right\vert x,y \in M, x \neq y \right\}.$$
...
Я бы сделал этот интеграл так
\int e^{e^x} e^{2014x} dx = \int e^{e^x} e^{2013x} de^x
После замена переменных e^x = y
\int e^y y^{2013} dy
Теперь задача посчитать интеграл вида 
\int e^y y^n dy
Это делается индуктивно пользуясь интегрированием по частям
\int e^y y^n dy = \int y^n d e^y = y^n e^y - \int e^y (y^n)' dy
Продолжая аналогично, получаем формулу
e^y[ \sum_{k=0}^n (-1)^k n!/(n-k)! y^{n-k} ]
Теперь надо подставить y = e^x и готово. 
Может быть эту формулу можно свернуть в более приличный вид. Но я пока не придумал (и не думал, если честно).
...
Первую я уже как-то объяснял. Напомню в чем суть. В пространстве матриц есть скалярное произведение (A, B) = tr(A^tB) = tr(AB^t). В этих терминах матрицы со следом ноль -- это ортогональное дополнение к <E> -- пространству скалярных матриц. Просят нас найти ортогональное дополнение к матрицам следа ноль, то есть (<E>^\bot)^\bot = <E>. Вот и все.
...
а) Докажите, что во множестве отрезков $\Lambda = \{ [i,j] \vert i,j=1,\ldots ,n, i<j \}$ можно
...
<https://medium.com/@mathreshka/hive-80-4ed6b54fae17>
<https://medium.com/@mathreshka/solution-hive-80-337e590b064f>

Обозначим F(x) множество всех друзей пользователя x (для определённости считаем, что x \in F(x))
...
Подстановка $\sigma$ задана двумя массивами $a[1\,.\,.\,n]$ и $b[1\,.\,.\,n]$ состоящими из всех различных чисел от $1$ до $n$ и такими, что $b[i] = \sigma (a[i])$ для каждого $i = 1,\ldots,n$ (например, $a = [2,\,\,3,\,\,1]$, $b = [1,\,\,3,\,\,2]$ кодирует транспозицию $(1,2)$). Придумайте алгоритм, определяющий, содержит ли $\sigma$ цикл длины $k$. Ваш алгоритм может изменять исходные массивы, но должен справляться с задачей за $O(n^2)$ операций с использованием $O(1)$ дополнительной памяти (оценивая эти две асимптотики, можете считать $k$ константой).
...
$A^T B - B^T A = A^T B - B^T A - E + E = (A + B)(B^T - A^T)$
...
### Камни -- [se+ ss- sl+ algorithms]

Есть куча из n камней. Два игрока играют в игру. Первый игрок на своем ходу может взять либо a1, либо a2, ... либо ak камней. Второй игрок на своем ходу может взять либо b1, либо b2, ... либо bm камней. Проиграет тот, кто не сможет сделать ход. Как можно определить, кто выиграет при оптимальной игре? И что значит оптимальная игра?

<http://math.hashcode.ru/questions/176845/теория-игр-теория-игр>
